...
const TARGET = 'packages/providers/src/catalog.ts';
...
type ProviderId = 'openai' | 'anthropic' | 'google' | 'openrouter' | 'opencode';
...
function toTs(catalog: Record<ProviderId, { models: ModelInfo[] }>) {
	...
	const imports = `import type { ProviderId, ModelInfo } from '@agi-cli/types';\n`;
	const bodyObject = JSON.stringify(catalog, null, 2);
	const body = `export const catalog: Record<ProviderId, { models: ModelInfo[] }> = ${bodyObject} as const satisfies Record<ProviderId, { models: ModelInfo[] }>;\n`;
	return `${header}\n${imports}\n${body}`;
}

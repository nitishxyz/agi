...
function loadKeypair(keypairPath?: string): Keypair {
  const resolvedPath = keypairPath || process.env.KEYPAIR_PATH || './test-wallet.json';
...
function createAuthHeaders(wallet: WalletAdapter): Record<string, string> {
  ...
  const signature = signMessage(Keypair.fromSecretKey(wallet.secretKey), message);
...
async function main() {
  ...
  await chatOpenAI(wallet, 'Count from 1 to 5, one number per line.', 'gpt-4o-mini', true);

  // Test Anthropic (non-streaming)
  console.log('\n--- Testing Anthropic (Claude) ---');
  await chatAnthropic(wallet, 'What is the capital of France? Answer in one word.', 'claude-3-5-haiku-latest', false);

  // Test Anthropic (streaming)
  console.log('\n--- Testing Anthropic streaming ---');
  await chatAnthropic(wallet, 'List 3 colors, one per line.', 'claude-3-5-haiku-latest', true);

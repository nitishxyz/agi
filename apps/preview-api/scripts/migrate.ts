...
const ACCOUNT_ID = process.env.CLOUDFLARE_ACCOUNT_ID || '861f82c965197980de430263380bad71';
...
async function migrate() {
	...
	if (!db) {
		...
		console.log('Available databases:', databases.map((d: any) => `${d.name} (${d.uuid})`).join('\n'));
		process.exit(1);
	}

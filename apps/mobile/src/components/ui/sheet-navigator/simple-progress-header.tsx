	...
	return (
		...
		>
			{Array.from({ length: totalSteps }, (_, index) => {
				...
				const isCompleted =
					index < currentStep ||
					(index === currentStep && isCurrentStepComplete);
...
const ProgressDot = ({
	...
	return (
		<Animated.View
			style={[styles.dot, { backgroundColor: getBackgroundColor() }, dotStyle]}
		>
			{isCompleted ? (
...
const ConnectingLine = ({
	...
	const lineStyle = useAnimatedStyle(() => {
		return {
			backgroundColor: withSpring(isCompleted ? '#10B981' : '#E5E7EB', {
				damping: 15,
				stiffness: 200,
			}),
		};

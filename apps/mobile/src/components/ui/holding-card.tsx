...
const HoldingCard: React.FC<HoldingCardProps> = ({ holding, width }) => {
				...
				style={{
					...
					borderColor: width
						...
						: Math.abs(holding.sessionPnlPercentage) > 0
							? holding.sessionIsPositive
								? '#10B981'
								: '#EF4444' // Use P&L color for full-width cards
				...
				{width ? (
					...
					>
							...
							<Box style={{ flex: 1 }}>
								...
								>
									<Box>
										...
										<Caption mode="subtle">
											{holding.tokenQuantity.toLocaleString()}{' '}
											{holding.tokenSymbol}
										</Caption>
									</Box>
						...
						>
							...
							>
								{holding.sessionIsPositive ? '+' : ''}$
								{Math.abs(holding.sessionPnlUsd).toFixed(2)} (
								{holding.sessionIsPositive ? '+' : ''}
								{holding.sessionPnlPercentage.toFixed(2)}%)
							</Body>
						</Box>
					</Animated.View>
				) : (

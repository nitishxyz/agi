			...
			<div className="flex items-center gap-1.5 pt-1">
				{running ? (
					<>
						...
						>
							{loading === 'stop' ? <Loader2 size={11} className="animate-spin" /> : <Square size={11} />}
							Stop
						</button>
					</>
				) : (
						...
						<>
							...
							<div className="absolute right-0 top-8 z-20 w-40 rounded-md border border-border bg-popover shadow-lg py-1">
								<MenuItem
									...
									label="Restart otto"
									onClick={() => { handleAction('restart'); setMenuOpen(false); }}
								/>
								<MenuItem
									icon={<ArrowUpCircle size={12} />}
									label="Update otto"
									onClick={() => { handleAction('update'); setMenuOpen(false); }}
								/>
								<MenuItem
									icon={<Download size={12} />}
									label="Export .otto"
									onClick={() => { handleAction('export'); setMenuOpen(false); }}
								/>
								...
								<MenuItem
									...
									label="Nuke"
									destructive
									onClick={() => { handleAction('nuke'); setMenuOpen(false); }}
								/>
							</div>
						</>
					)}

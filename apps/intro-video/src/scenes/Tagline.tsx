...
export const Tagline: React.FC = () => {
  ...
  const line1 = spring({ frame: frame - 5, fps, config: { damping: 14, mass: 1, stiffness: 80 } });
  const line1Y = interpolate(line1, [0, 1], [80, 0]);
  const line1Rotate = interpolate(line1, [0, 1], [3, 0]);

  const line2 = spring({ frame: frame - 22, fps, config: { damping: 14, mass: 1, stiffness: 80 } });
  const line2Y = interpolate(line2, [0, 1], [80, 0]);
  const line2Rotate = interpolate(line2, [0, 1], [-2, 0]);

  const descProgress = spring({ frame: frame - 50, fps, config: { damping: 20, stiffness: 70 } });
  const descY = interpolate(descProgress, [0, 1], [40, 0]);

  const pillProgress = spring({ frame: frame - 70, fps, config: { damping: 14, stiffness: 180 } });
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8, transform: `translateY(${floatY}px)` }}>
        ...
        >
          An open-source AI assistant that reads, writes,
          and executes — right from your terminal.
        </div>

        <div style={{ display: "flex", gap: 14, marginTop: 24 }}>
          {["Open-source", "Provider-agnostic", "Extensible"].map((tag, i) => {
            const tagDelay = 75 + i * 10;
            const tagProgress = spring({ frame: frame - tagDelay, fps, config: { damping: 14, stiffness: 200 } });

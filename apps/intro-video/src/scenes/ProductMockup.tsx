...
const TOOL_STEPS = [
	{
		icon: 'search',
		label: 'glob',
		detail: 'src/**/*.tsx — 8 files',
		color: colors.amber,
	},
	{
		icon: 'file',
		label: 'read',
		detail: 'src/App.tsx — 42 lines',
		color: '#3B82F6',
	},
	{
		icon: 'write',
		label: 'write',
		detail: 'src/Waitlist.tsx — 64 lines',
		color: colors.emerald,
	},
	{
		icon: 'patch',
		label: 'apply_patch',
		detail: 'App.tsx — 4 lines changed',
		color: colors.purple,
	},
	{
		icon: 'terminal',
		label: 'terminal',
		detail: 'bun dev — ready on :3000',
		color: colors.muted,
	},
];

const ToolIcon: React.FC<{ type: string; color: string; size?: number }> = ({
	type,
	color,
	size = 20,
}) => {
						...
						{TOOL_STEPS.map((step, i) => {
							...
							return (
								...
								>
									...
									<span
										style={{ fontSize: 22, fontWeight: 600, color: step.color }}
									>
										{step.label}
									</span>
									<span style={{ fontSize: 20, color: colors.dim }}>
										{step.detail}
									</span>
								</div>
							);
					...
					{(() => {
						...
						return (
							...
							>
								...
								<div
									style={{ fontSize: 22, color: colors.text, lineHeight: 1.5 }}
								>
									Created{' '}
									<code
										style={{
									...
									with animated mesh gradient and confetti on submit.
								</div>
							</div>
						);
					})()}

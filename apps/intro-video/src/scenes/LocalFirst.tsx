...
const PlugIcon: React.FC<{ size: number; color: string }> = ({ size, color }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
    <path d="M12 22v-5" />
    <path d="M9 8V2" />
    <path d="M15 8V2" />
...
const ItemIcon: React.FC<{ type: string; size: number; color: string }> = ({ type, size, color }) => {
  switch (type) {
    case "terminal": return <TerminalIcon size={size} color={color} />;
    case "database": return <DatabaseIcon size={size} color={color} />;
    case "plug": return <PlugIcon size={size} color={color} />;
    default: return null;
  }
};
...
export const LocalFirst: React.FC = () => {
  ...
  const titleProgress = spring({ frame: frame - 5, fps, config: { damping: 16, mass: 0.9, stiffness: 80 } });
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <div style={{ fontSize: 22, color: colors.dim, letterSpacing: "0.3em", textTransform: "uppercase" as const, marginBottom: 20, fontFamily: font.mono, fontWeight: 500 }}>
            How it works
          </div>
          <div style={{ fontSize: 88, fontWeight: 700, color: colors.text, letterSpacing: "-0.03em", lineHeight: 1.05 }}>
            Runs where
            <br />
            you do.
          </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 20, flex: 1 }}>
          {ITEMS.map((item, i) => {
            const delay = 20 + i * 18;
            const progress = spring({ frame: frame - delay, fps, config: { damping: 14, mass: 0.7, stiffness: 140 } });
            ...
            return (
              <div
                ...
                style={{
                  ...
                  borderRadius: 22,
                  padding: "28px 32px",
                  boxShadow: "0 4px 24px rgba(0,0,0,0.03), 0 1px 6px rgba(0,0,0,0.02)",
                }}
              >
                <div
                  style={{
                ...
                >
                  <ItemIcon type={item.icon} size={36} color={colors.accent} />
                </div>
                <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                  <span style={{ fontSize: 28, fontWeight: 700, color: colors.text }}>
                    {item.title}
                  </span>
                  <span style={{ fontSize: 20, color: colors.muted, lineHeight: 1.5 }}>
                    {item.desc}
                  </span>
                </div>
              </div>

...
function writeWav(name: string, samples: Float32Array) {
  ...
  console.log(`  ✓ ${name}.wav (${(buf.length / 1024).toFixed(1)}KB, ${(n / RATE).toFixed(2)}s)`);
}
...
// 7. Tick — gentle tap, like a pebble in water
{
  ...
  for (let i = 0; i < samples.length; i++) {
    ...
    const s = sine(freq * t) * 0.4 + sine(freq * 1.5 * t) * 0.1 * Math.exp(-t * 30);
...
// 8. Success — two gentle glass tones, ascending
{
  ...
  for (let i = 0; i < samples.length; i++) {
    ...
    const f1 = 392;
    const f2 = 523.25;
    const s = (sine(f1 * t) * 0.3 + sine(f1 * 2 * t) * 0.08) * a1
            + (sine(f2 * t) * 0.35 + sine(f2 * 2 * t) * 0.1 + sine(f2 * 3 * t) * 0.03) * a2;
    samples[i] = s * 0.45;
  }

...
const CommitMessageSchema = z.object({
  type: z.enum(['feat', 'fix', 'docs', 'style', 'refactor', 'perf', 'test', 'build', 'ci', 'chore']),
  ...
  breakingDescription: z.string().optional().describe('Description of breaking changes')
});
...
async function getGitDiff(): Promise<string> {
  try {
    ...
    if (unstaged.trim()) {
      console.log('ℹ️  No staged changes found, analyzing unstaged changes...\n');
      return unstaged;
    }
    
    throw new Error('No changes found. Stage some changes with `git add` or make some changes.');
  } catch (error) {
...
async function main() {
  ...
  try {
    ...
    if (diff.length > 10000) {
      console.warn('⚠️  Warning: Diff is very large, this may take a moment...\n');
    }

...
export function resolveModel(
	...
	if (provider === 'anthropic' && cfg.providers.anthropic?.apiKey && !process.env.ANTHROPIC_API_KEY) {
		process.env.ANTHROPIC_API_KEY = cfg.providers.anthropic.apiKey;
	}
	if (provider === 'google' && cfg.providers.google?.apiKey && !process.env.GOOGLE_GENERATIVE_AI_API_KEY) {

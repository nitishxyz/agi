import { intro, outro, select, multiselect, text, isCancel, cancel } from '@clack/prompts';
...
export async function runSetup(projectRoot?: string) {
  ...
  for (const p of Object.keys(providers) as ProviderId[]) {
    ...
    const keyLabel = p === 'openai' ? 'OPENAI_API_KEY' : p === 'anthropic' ? 'ANTHROPIC_API_KEY' : 'GOOGLE_GENERATIVE_AI_API_KEY';
    const key = await text({ message: `Enter ${keyLabel} (leave empty to skip)`, initialValue: '' });
  ...
  const configPath = cfg.paths.projectConfigPath || `${cfg.paths.dataDir}/config.json`;

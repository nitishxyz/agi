...
const ESC = '\u001b[';
function code(n: number) { return `${ESC}${n}m`; }
...
export function table(headers: string[], rows: string[][]) {
  ...
  for (const r of all) {
    for (let i = 0; i < cols; i++) widths[i] = Math.max(widths[i], stripAnsi(r[i] ?? '').length);
  }
  const sep = '  ';
  console.log(headers.map((h, i) => colors.bold(pad(h, widths[i]))).join(sep));
  console.log(widths.map((w) => '─'.repeat(w)).join(sep));
  for (const r of rows) console.log(r.map((c, i) => pad(c ?? '', widths[i])).join(sep));
}
...
function stripAnsi(s: string) {
  ...
  return s.replace(/[\u001B\u009B][[\]()#;?]*(?:[0-9]{1,4}(?:;[0-9]{0,4})*)?[0-9A-ORZcf-nq-uy=><]/g, '');
}

...
export const SessionHeader: React.FC<SessionHeaderProps> = ({ session }) => {
	...
	const calculateCost = () => {
		...
		const inputCost =
			(session.tokenUsage.inputTokens / 1_000_000) * inputCostPer1M;
		const outputCost =
			(session.tokenUsage.outputTokens / 1_000_000) * outputCostPer1M;
					...
					<div className="flex items-center gap-2">
						...
						<span>
							...
							<span className="text-xs ml-1">
								({formatNumber(session.tokenUsage.inputTokens)} in /{' '}
								{formatNumber(session.tokenUsage.outputTokens)} out)
							</span>
						</span>
					</div>

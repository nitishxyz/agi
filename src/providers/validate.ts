...
export function validateProviderModel(provider: string, model: string, cap?: CapabilityRequest) {
  ...
  if (!entry) {
    const list = catalog[p].models.slice(0, 10).map((m) => m.id).join(', ');
    throw new Error(`Model not found for provider ${provider}: ${model}. Example models: ${list}${catalog[p].models.length > 10 ? ', ...' : ''}`);
  }
  ...
  if (cap?.wantsVision) {
    ...
    if (!ok) throw new Error(`Model ${model} does not support vision input/output.`);
  }
}

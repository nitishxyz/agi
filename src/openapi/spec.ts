    ...
    paths: {
      ...
      '/v1/sessions/{id}/messages': {
        ...
        post: {
            ...
            content: {
              'application/json': {
                schema: {
                  ...
                  properties: {
                    content: { type: 'string' },
                    agent: { type: 'string', description: 'Agent name. Defaults to config if omitted.' },
                    provider: { $ref: '#/components/schemas/Provider' },
                    model: { type: 'string' }
                  }
                }
          ...
          responses: {
            202: { description: 'Accepted', content: { 'application/json': { schema: { type: 'object', properties: { messageId: { type: 'string' } }, required: ['messageId'] } } } },
            400: errorResponse()
          }
        }
      },
        ...
        get: {
          ...
          responses: {
            200: {
              ...
              content: {
                'text/event-stream': {
                  schema: { type: 'string', description: 'SSE event stream. Events include session.created, message.created, message.part.delta, tool.call, tool.delta, tool.result, message.completed, error.' }
                }
              }
            }
          }
      ...
      schemas: {
        ...
        MessagePart: {
          ...
          properties: {
            ...
            type: { type: 'string', enum: ['text', 'tool_call', 'tool_result', 'image', 'error'] },
...
function withoutParam() {
  return {
    ...
    description: 'Exclude parts from the response. By default, parts are included.'
  } as const;
}

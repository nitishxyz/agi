...
const inputSchema = z.object({
	limit: z.number().min(1).max(100).default(20).describe('Max sessions to return'),
	offset: z.number().min(0).default(0).describe('Offset for pagination'),
	agent: z.string().optional().describe('Filter by agent type'),
	startDate: z.string().optional().describe('Filter sessions created after this ISO date'),
	endDate: z.string().optional().describe('Filter sessions created before this ISO date'),
	orderBy: z.enum(['created_at', 'last_active_at', 'total_tokens']).default('last_active_at').describe('Sort field'),
	orderDir: z.enum(['asc', 'desc']).default('desc').describe('Sort direction'),
	sessionType: z.enum(['main', 'research', 'all']).default('main').describe('Filter by session type'),
});
...
export function buildQuerySessionsTool(projectRoot: string) {
	return {
		name: 'query_sessions',
		tool: tool({
			description: 'Search and list sessions from the local database. Use to find past conversations, check what work was done, or locate specific sessions.',
			inputSchema,
			async execute(input) {
				...
				const orderField = input.orderBy === 'created_at' 
					? sessions.createdAt 

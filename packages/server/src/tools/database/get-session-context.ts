...
const inputSchema = z.object({
	...
	includeMessages: z.boolean().default(false).describe('Include full message content'),
	messageLimit: z.number().min(1).max(100).default(50).describe('Max messages to include if includeMessages is true'),
});
...
export function buildGetSessionContextTool(projectRoot: string) {
	return {
		...
		tool: tool({
			description: 'Get detailed context for a specific session including summary, stats, and optionally full messages. Use to understand what happened in a past conversation.',
			inputSchema,
			async execute(input) {
